// 조디악 블리츠(ZODIAC BLITZ) 온라인 서버 (서버 권위, 매치메이킹 큐 기반).
...
const httpServer = http.createServer((req, res) => {
  ...
  if (!fs.existsSync(file)) {
    ...
    res.end('조디악 블리츠 서버 동작 중. 앱은 `npm run build` 후 이 포트로, 개발 중엔 vite(5173)로 접속하세요.')
    return
  }
...
httpServer.listen(PORT, '0.0.0.0', () => {
  console.log(`[zodiac-blitz] 서버 시작: http://0.0.0.0:${PORT} (ws: /ws)`)
})
