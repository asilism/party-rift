...
test('콜로세움: 차감은 2,4,6… 포인트 0이면 탈락', () => {
  assert.equal(arenaDeduction(1), 2)
  assert.equal(arenaDeduction(2), 4)
  assert.equal(arenaDeduction(3), 6)
  const tour = createTournament('tiger', 'warrior', lcg(2))
  const rng = lcg(3)
  // 유저가 계속 지면: 10-2=8 → 8-4=4 → 4-6=0(탈락, 3라운드째)
  for (let r = 1; r <= 3; r++) {
    nextRound(tour, rng)
...
test('콜로세움: 4팀+ 생존이면 직전 라운드와 같은 대진이 반복되지 않는다', () => {
  const pairKey = (p) => [p[0].idx, p[1].idx].sort((x, y) => x - y).join('-')
  for (let seed = 100; seed < 140; seed++) {
    const rng = lcg(seed)
    const tour = createTournament('horse', 'archer', rng)
    let prev = null
    let guard = 0
    while (!tour.over && guard++ < 30) {
      const cur = nextRound(tour, rng)
      const alive = tour.teams.filter((tm) => tm.alive).length
      if (prev && alive >= 4) {
        const keys = new Set(cur.pairs.map(pairKey))
        for (const k of prev) assert.ok(!keys.has(k), `seed ${seed} r${tour.round}: 대진 ${k} 반복`)
      }
      prev = new Set(cur.pairs.map(pairKey))
      resolveRound(tour, rng() < 0.5, rng)
    }
  }
})
