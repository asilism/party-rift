...
test('엔지니어 포탑 재고: 쿨마다 1개 충전(최대 3), 재고가 있으면 즉시 연속 설치', () => {
  ...
  startPlaying(g)
  const en = g.heroes[0]
  en.x = en.homeX // 자기 우물 — 긴 충전 시간 동안 미니언 웨이브에 휘말리지 않게
  en.z = en.homeZ
  step(g, STEP)
...
test('미니언도 소환물(포탑)을 표적으로 삼는다', () => {
  ...
  run(g, 1.0)
  assert.ok(t.hp < 400, '적 미니언이 포탑을 공격한다')
})
...
test('경험치: 막타 없이도 근처(XP_RANGE) 아군 영웅이 미니언 경험치를 받는다 — 막타는 골드에만 필요', () => {
  const g = createGame(humans())
  startPlaying(g)
  ...
  const xp0 = h.xp
  const gold0 = h.gold
  // 아군(blue) 미니언이 빈사 적(red) 미니언을 막타 — 영웅은 손도 안 댄다
  plantMinion(g, 'red', 40, 0, 10)
  plantMinion(g, 'blue', 41.5, 0, 300)
  run(g, 1.5)
  assert.ok(!g.minions.some((m) => m.team === 'red' && m.x === 40), '적 미니언이 아군 미니언에게 죽었다')
...
test('봇 웨이브 정리: 적 미니언이 뭉쳐 있으면 스킬(화염구)로 정리한다', () => {
  const g = createGame(humans())
  startPlaying(g)
  ...
  bot.x = 0
  bot.z = 0
  // 사거리 안에 뭉친 적 미니언 4마리 (적 영웅은 전부 우물 — 교전 없음)
...
test('수비 콜: 영웅 없이 큰 미니언 웨이브만 타워를 갉아먹어도 봇이 막으러 온다', () => {
  const g = createGame(humans())
  startPlaying(g)
...
test('봇 공성: 미니언 방패가 받아주는 동안 평타를 미니언이 아니라 타워에 꽂는다', () => {
  const g = createGame(humans())
  startPlaying(g)
  const bot = makeBot(g, g.heroes[0].id) // blue 마법사
  bot.role = 'mid'
  ...
  bot.x = tower.x - 10
  bot.z = tower.z
  // 아군 미니언 방패(타워 사거리 안) + 봇에게 더 가까운 적 미니언(예전엔 이 녀석만 팼다)
...
test('공포술사 공포의 시선: 2초 공포 + 도주 속도가 기어가는 수준으로 느리다', () => {
  const g = duo('fearmonger', 'tank')
  startPlaying(g)
  g.minions.length = 0
  ...
  t.x = 4; t.z = 0
  castSkill(g, f.id)
  assert.ok(t.fearT >= 1.9, `공포가 2초 지속된다 (${t.fearT})`)
  const x0 = t.x
  run(g, 1)
  const moved = t.x - x0
  assert.ok(moved > 0.5, '공포 중엔 시전자 반대로 도망친다')
  assert.ok(moved < CLASSES.tank.speed * 0.5, `거의 못 움직인다 — 1초에 ${moved.toFixed(1)} < ${(CLASSES.tank.speed * 0.5).toFixed(1)}`)
})
...
test('공포술사 단말마(리메이크): 적에게 순간이동해 주변 전원을 2초 공포에 빠뜨린다', () => {
  const g = createGame([
  ...
  assert.ok(t1.fearT >= 1.9 && t2.fearT >= 1.9, '주변 적 전원이 2초 공포')
...
test('훈련장(sandbox): 웨이브·정글몹·타워가 멈춘 조용한 맵', () => {
  const g = createGame(humans(), { sandbox: true, rng: () => 0.5 })
  startPlaying(g)
  const h = g.heroes[0] // blue
  // 적 타워 코앞에 서 있어도 맞지 않는다
  const redTower = g.towers.find((t) => t.team === 'red')
  h.x = redTower.x + 2
  h.z = redTower.z
  run(g, 5)
  assert.equal(g.minions.length, 0, '미니언 웨이브가 나오지 않는다')
  assert.ok(!g.monsters.find((m) => m.kind === 'dragon').alive, '용도 등장하지 않는다')
  assert.equal(h.hp, h.maxHp, '타워가 쏘지 않는다')
})
