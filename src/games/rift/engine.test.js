...
// ── P3: 검성 / 사슬잡이 ──
test('검성 발도 카운터: 다음 피해 1회를 막고 반격한다', () => {
  ...
  assert.ok(as.hp < asHp, '공격자에게 반격이 들어간다')
})
...
test('엔지니어 미니포탑: 최대 3기 · 자동 사격 · 평타 2대면 파괴', () => {
  ...
  const t0 = turrets[0]
  w.x = t0.x + 2; w.z = t0.z // 전사가 포탑 바로 옆
  castAttack(g, w.id); run(g, 0.7) // 평타 1대
  assert.ok(g.summons.includes(t0) && t0.hp < 110, '1대로는 안 터지고 체력만 깎임')
  castAttack(g, w.id); run(g, 0.4) // 평타 2대
  assert.ok(!g.summons.includes(t0), '평타 2대면 파괴(저체력)')
})
...
test('미니포탑 휴면: 주인이 사거리 밖이면 잠들어(zzz) 사격을 멈춘다', () => {
  ...
  en.x = 40; en.z = 40 // 주인이 사거리 밖으로 이탈
  run(g, 0.2)
  assert.equal(!!t.dormant, true, '주인이 사거리 밖이면 휴면')
  const hp2 = foe.hp
  run(g, 1.0)
