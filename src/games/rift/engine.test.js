...
test('전사 회전베기: 도는 동안 받는 피해가 크게 줄어든다(앞라인 탱킹)', () => {
  ...
  const w = g.heroes[0]
  w.lvl = ULT_LEVEL
  // 적 우물 피해로 비교 (같은 위치·시간, 궁극기 OFF/ON) — 최후의 포탑 사거리 밖에 둔다
  w.x = NEXUS_POS.red.x; w.z = NEXUS_POS.red.z + 11
  run(g, 1)
...
test('용 분노: 12레벨이면 혼자서도 용을 잡는다(쉽지 않게)', () => {
  ...
  setLevel(h, 12)
  // 밸런스 패치로 공격력 곡선이 20% 낮아져 맨몸 12레벨로는 분노에 밀린다 —
  //  용을 노릴 무렵이면 보통 무기 하나는 갖추므로 기본 무기를 들려 검증한다.
  h.items = ['longsword']
...
test('아이템 효과: 공격력/방어 아이템이 실제 전투 수치에 반영된다', () => {
  ...
  // 평타 피해 = 고유 공격력(곡선 20%↓ 반영) + 아이템 공격력 → 아이템 단독값보다 분명히 크다
  assert.ok(dmg >= itemAtk + 30, '공격력 아이템(배율 포함)이 탄 피해에 더해진다')
})
