...
export const riftNet = {
  ...
  buildParticipants(players, config) {
    ...
    if (Array.isArray(config?.roster) && config.roster.length) {
      ...
      return { players: list, opts: { mode, botLevel: config?.botLevel, bossTier: config?.bossTier, carry: config?.carry, arenaLayout: config?.arenaLayout, arenaPts: config?.arenaPts, arenaDeduct: config?.arenaDeduct, arenaRound: config?.arenaRound, bossRush: config?.bossRush, ultDebug: config?.ultDebug } }
    }
  ...
  createGame: (players, opts) =>
    createGame(players, { mode: opts?.mode, botLevel: opts?.botLevel, bossTier: opts?.bossTier, carry: opts?.carry, arenaLayout: opts?.arenaLayout, arenaPts: opts?.arenaPts, arenaDeduct: opts?.arenaDeduct, arenaRound: opts?.arenaRound, bossRush: opts?.bossRush, ultDebug: opts?.ultDebug, rng: Math.random }),
  setInput,
  ...
  predictLocal(pred, ctrl, me, dt) {
    ...
    if (me.respawnT > 0 || me.stunT > 0 || me.castT > 0 || me.recallT > 0 || me.hookWindT > 0 || me.knockT > 0 || me.fearT > 0 || me.charmT > 0) return // 매혹 중엔 로컬 예측도 멈춘다(몽마)
