...
export const CLASSES = {
  ...
  swordmaster: {
    ...
    ult: { name: '무형검', icon: '🌀', cd: 60, desc: '수 초간 사거리·공격속도가 크게 오르고, 평타마다 초승달 검기를 날려 직선의 적을 모두 벤다' },
  },
  catcher: {
  ...
  fearmonger: {
    ...
    ult: { name: '단말마', icon: '💀', cd: 60, desc: '주변 모든 적을 2초 공포에 빠뜨리고, 달아나는 동안 어둠이 갉아먹는다(도트)' },
  },
  illusionist: {
...
export const ABILITY_SCALING = {
  ...
  mage: { skill: { dmg: [75, 1.1] }, skill2: { dmg: [45, 0.7], note: '점프마다 약화 · 최대 3타' }, ult: { dmg: [60, 0.9], note: '운석 3발' } },
  ...
  swordmaster: { skill: { dmg: [130, 1.5], note: '1.5초 자세 · 받은 피해 1회 무효 + 2배 반사(반사 상한)' }, ult: { note: '5초 · 사거리 +4 · 공속 +45% · 평타가 관통 검기가 된다' } },
  ...
  fearmonger: { skill: { dmg: [30, 0.55], note: '2초 공포(거의 정지 수준으로 느려져 도주)' }, skill2: { shield: [45, 0.7], note: '가속 + 어둠 장막' }, ult: { dmg: [40, 0.6], dot: [10, 0.18], dotDur: 2, note: '주변 전체 2초 공포' } },
...
const ROCK_HIT_R = 2.0 // 직격 판정 반경(영웅 기준 — 미니언은 -0.2, 정글몹은 +0.6)
...
function stepProjectiles(state, dt) {
  ...
  for (const p of state.projectiles) {
    ...
    if (p.kind === 'rock') {
      // 대지술사 돌덩이: 직선 비행 — 첫 적(영웅 > 미니언 > 정글몹)에 직격하면
