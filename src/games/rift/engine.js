...
function bossThink(state, h, dt) {
  ...
  if (h.bossBeamAt && state.time >= h.bossBeamAt) {
    ...
    const bx = Math.cos(h.bossBeamDir)
    const bz = Math.sin(h.bossBeamDir)
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
      const rx = e.x - h.x
      const rz = e.z - h.z
      ...
      applyKnockback(state, e, h.x, h.z, 30, 1.2) // 빛의 격류에 날아간다
      e.beamDeathAt = state.time + 0.55 // 날아가던 끝에서 소멸 — 맵 끝 데스 연출
...
function bossArchmage(state, h, foe) {
  ...
  if (!h.defenseBoss && (h.bossCd.stones ?? 1) <= 0
    ...
    for (let i = 0; i < count; i++) {
      ...
      state.minions.push({
        ...
        hp: STONE_HP, maxHp: STONE_HP, atkCd: 0, wpI: 0, dir: 0, atkSeq: 0,
      })
    }
  ...
  if (!h.defenseBoss && h.bossCd.d <= 0 && !h.bossBeamAt) {
    ...
    if (bt) {
      ...
      pushFeed(state, 'obj', '☀️ 섬멸의 광선 — 빛의 경로에서 벗어나라! 맞으면 소멸한다!')
      return
    }
  }
  ...
  // 바위감옥 + 운석 3연타(대지술사+마법사 연계): 표적을 돌기둥 링에 가두고 유저 운석의
  //  몇 배짜리 대형 운석을 세 번 내리꽂는다. 감옥은 걸어서 못 나간다 — 돌진·점멸로 탈출하거나
  //  치유·보호막으로 버텨라(트인 카운터플레이 둘).
  if (h.bossCd.a <= 0 && foe && dist(h, foe) < 22) {
    h.bossCd.a = PRISON_CD * cdMul
    raiseWallRing(state, foe.x, foe.z, 3.4, 9, 3.6)
    foe.rootT = Math.max(foe.rootT, 0.9) // 갇히는 순간 발이 묶인다 — 이후는 벽이 막는다
    for (let i = 0; i < 3; i++) {
      state.zones.push({
        id: state.nextId++, kind: 'meteor', team: h.team, owner: h.id,
        x: foe.x, z: foe.z, r: 5, t: 0, delay: 1.2 + i * 0.7, dmg: skillDmg(h, 130, 2.5),
      })
    }
    pushFx(state, 'rocksplash', foe.x, foe.z, 4, h.team, 0.8)
    pushFeed(state, 'obj', `🪨 ${foe.name}이(가) 바위감옥에 갇혔다 — 운석이 떨어진다! 구출하거나 버텨라!`)
  }
}
...
function botDodgeBeam(state, h, dt) {
  for (const b of state.heroes) {
    if (!b.isBoss || b.team === h.team || !b.bossBeamAt || b.respawnT > 0) continue
    const bx = Math.cos(b.bossBeamDir)
    const bz = Math.sin(b.bossBeamDir)
    const rx = h.x - b.x
    const rz = h.z - b.z
    const along = rx * bx + rz * bz
    const perp = -rx * bz + rz * bx
    if (along < -2 || along > BEAM_LEN + 2 || Math.abs(perp) > BEAM_HALF + 2.2) continue
    const side = perp >= 0 ? 1 : -1 // 이미 치우친 쪽으로 마저 빠진다(경로 최단)
    steerToward(state, h, {
      x: b.x + bx * Math.max(2, along) - bz * side * (BEAM_HALF + 5),
      z: b.z + bz * Math.max(2, along) + bx * side * (BEAM_HALF + 5),
    })
    botAttack(state, h, dt)
    return true
  }
