...
const BRAWL_GUARD_T = 2 // 리스폰 직후 무적(스폰킬 방지) — 2초 깜빡(유저 튜닝 2026-08-03)
...
const heroSpeed = (h) => (CLASSES[h.cls].speed + itemBonus(h).speed + augOf(h).speed + trophyFx(h).speed) * ((h.brawlStarT || 0) > 0 ? 2 : 1) * ((h.brawlFrogT || 0) > 0 ? 0.62 : 1) // 🐔 닭 걸음 / ⭐ 별 2배 / 🐸 개구리 굼뜸
...
export function createGame(players, opts = {}) {
  ...
  for (const h of heroes) {
    ...
    //  녹스는 티어 가중을 55%만 — 기본 체력을 올려(야바위 개편 쉬움 보정) 티어까지 복리로
    //  불면 악몽·지옥이 절벽이 된다(실측 31%→13% 추락). 실효 체력은 개편 전과 동급으로 유지.
    if (h.isBoss) {
      const thp = BOSS_TIERS[o.bossTier]?.hp || 1
      const hpMul = h.cls === 'boss_shadow' ? 1 + (thp - 1) * 0.15 : thp // 주의: 더 눅이면 P2/P3(포위·다단) 진입이 빨라져 오히려 어려워진다
      h.maxHp = Math.round(h.maxHp * hpMul)
    }
  ...
  const monsters = [
    ...
    }),
    {
      ...
      hp: DRAGON.hp, maxHp: DRAGON.hp, alive: false, respawnT: mode === 'arena' || mode === 'brawl' ? 1e9 : DRAGON.spawn, aggro: null,
    },
    {
      id: 'baron', kind: 'baron', camp: map.BARON_PIT, x: map.BARON_PIT.x, z: map.BARON_PIT.z,
      hp: BARON.hp, maxHp: BARON.hp, alive: false, respawnT: mode === 'arena' || mode === 'brawl' ? 1e9 : BARON.spawn, aggro: null,
    },
  ]
  return {
    ...
    kills: mode === 'brawl' ? Object.fromEntries(heroes.map((h) => [h.team, 0])) : { blue: 0, red: 0 }, // FFA는 고유 팀 키
...
// 바위감옥(아르케인): 표적 둘레를 돌기둥 링으로 가둔다 — 돌벽(tempWalls)과 같은 충돌 규칙이라
// 걸어서는 못 나가고, 돌진·점멸류 이동기로만 탈출한다(감옥의 카운터플레이).
function raiseWallRing(state, cx, cz, radius, count, life) {
  for (let i = 0; i < count; i++) {
    const a = (i / count) * Math.PI * 2
    state.tempWalls.push({
      id: state.nextId++, x: cx + Math.cos(a) * radius, z: cz + Math.sin(a) * radius, alive: true, t: 0, life,
    })
  }
}
...
const DREAM_R = 11.2 // 꿈의 영토 반경(0.8배 하향 — 그래도 한타를 삼킨다)
const DREAM_TIME = 2.5 // 꿈의 영토 지속(3.5초는 과했다)
...
export function castAttack(state, id, forceRef = null) {
  { const fr = getHero(state, id); if (fr && (fr.brawlFrogT || 0) > 0) return state } // 🐸 개구리는 개굴개굴만
  if (state.status !== 'playing') return state
  const h = getHero(state, id)
...
export function castUlt(state, id) {
  ...
  if (state.mode === 'brawl') {
    ...
    if (h.cls === 'mage') {
    ...
    } else if (h.cls === 'gladiator') {
      h.brawlTitanT = 6 // 🏛️ 투기장의 거인: 6초간 몸집 2배 + 모든 평타가 뿅망치(매 타 피니셔)
      pushFeed(state, 'obj', `🏛️ ${emojiOf(h.zodiacId)} ${h.name} — 거인이 되었다!`)
    } else if (h.cls === 'warlock') {
      // 🐸 대변이: 주변 적 전원을 3초간 개구리로 — 공격 불가·굼뜬 걸음(웃김 포인트)
      for (const e of state.heroes) {
        if (e.team === h.team || e.respawnT > 0 || e.hp <= 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0) continue
        if (Math.hypot(h.x - e.x, h.z - e.z) > 12) continue
        e.brawlFrogT = 3
      ...
      // ❄️ 대빙원: 전방 절반을 통째로 얼린다(20) + 얼음판 살포 — 밟으면 주르륵
      ...
      for (const e of state.heroes) {
        ...
        const dx = e.x - h.x
        const dz = e.z - h.z
        if (Math.hypot(dx, dz) > 20 || dx * cf + dz * cz < 0) continue
        e.freezeT = Math.max(e.freezeT, 3)
      }
      for (let k = 0; k < 8; k++) {
        const ba = h.dir + (state.rng() - 0.5) * 2.2
        const br = 4 + state.rng() * 13
      ...
      // ⛓️ 단죄: 사슬을 뻗어 주변 전원을 내 자리로 끌어온 뒤(하얀 사슬은 씬이 그린다) 일제 단죄
      const chained = []
      for (const e of state.heroes) {
        if (e.team === h.team || e.respawnT > 0 || e.hp <= 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0 || (e.brawlMushT || 0) > 0) continue
        const ed = Math.hypot(h.x - e.x, h.z - e.z)
        if (ed > 13 || ed < 1.2) continue
        applyKnockback(state, e, e.x * 2 - h.x, e.z * 2 - h.z, Math.min(ed - 1.2, 11))
        e.stunT = Math.max(e.stunT, 1.3)
        chained.push({ x: e.x, z: e.z })
      }
      state.brawlChainSeq = (state.brawlChainSeq || 0) + 1
      state.brawlChainAt = { x: h.x, z: h.z, targets: chained }
      h.brawlChainT = 0.9
...
function fireLightArrow(state, h) {
  ...
  h.revealT = Math.max(h.revealT, REVEAL_TIME)
  if (state.mode === 'brawl') {
    // 🌈 극태 레이저: 본래 궁의 빛줄기가 에너지 빔으로 진화 — 관통 전원 강넉백 + 본인 반동
    ...
    applyKnockback(state, h, h.x + lx, h.z + lz, 4) // 반동 — 뒤로 주르륵(웃김 포인트)
    state.brawlLaserSeq = (state.brawlLaserSeq || 0) + 1
    state.brawlLaserAt = { x: h.x, z: h.z, dir }
  }
}
...
function damageHero(state, victim, amount, attacker, redirected = false, tag = null, dot = false) {
  ...
  if (state.mode === 'brawl' && attacker && attacker.team !== victim.team && attacker.x != null
    ...
    if ((victim.brawlMushT || 0) > 0) {
      ...
      const hammer = (attacker.brawlHammerT || 0) > 0 || (attacker.brawlTitanT || 0) > 0 // 🔨 뿅망치·🏛️ 티탄: 매 타가 홈런
  ...
  if (killer) {
    ...
    if (state.mode !== 'brawl') state.kills[enemyOf(victim.team)]++ // FFA엔 '상대팀' 개념 없음
...
function stepBrawl(state, dt) {
  ...
  for (let i = state.healOrbs.length - 1; i >= 0; i--) {
    ...
    o.t += dt
    if (o.t > 20) { state.healOrbs.splice(i, 1); continue }
    for (const h of state.heroes) {
      if (h.respawnT > 0 || h.hp <= 0 || h.fallT > 0) continue
  ...
  // 보급 아이템 습득 — 즉발(⚡) 또는 일시 버프
  for (let i = state.brawlPickups.length - 1; i >= 0; i--) {
    const pk = state.brawlPickups[i]
    pk.t += dt
    if (pk.t > 18) { state.brawlPickups.splice(i, 1); continue }
    for (const h of state.heroes) {
      if (h.respawnT > 0 || h.hp <= 0 || h.fallT > 0) continue
      if (Math.hypot(h.x - pk.x, h.z - pk.z) > 2.0) continue
      if (pk.kind === 'hammer') {
        h.brawlHammerT = 10 // 🔨 뿅망치: 내 타격의 넉백이 크게 강해진다
        pushFeed(state, 'obj', `🔨 ${emojiOf(h.zodiacId)} ${h.name} — 뿅망치를 주웠다`)
      } else if (pk.kind === 'mush') {
        h.brawlMushT = 8 // 🍄 거대버섯: 몸이 커지고 잘 안 밀린다
        pushFeed(state, 'obj', `🍄 ${emojiOf(h.zodiacId)} ${h.name} — 거대해졌다`)
        ...
        h.brawlBananaN = 4 // 🍌 바나나 다발: 평타 한 대마다 껍질을 표적 발밑에 던진다
        pushFeed(state, 'obj', `🍌 ${emojiOf(h.zodiacId)} ${h.name} — 바나나 다발을 들었다`)
      } else if (pk.kind === 'gust') {
        for (const e of state.heroes) { // 💨 돌풍 부적: 주변(12) 적 전원을 바깥으로 강하게 밀쳐낸다 — 자석의 반대
          if (e === h || e.respawnT > 0 || e.hp <= 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0) continue
          const ed = Math.hypot(h.x - e.x, h.z - e.z)
          if (ed > 12) continue
          applyKnockback(state, e, h.x, h.z, 7.5)
        }
        pushFx(state, 'rocksplash', h.x, h.z, 6, null, 1.0)
        pushFeed(state, 'obj', `💨 ${emojiOf(h.zodiacId)} ${h.name} — 돌풍이 몰아친다!`)
      } else if (pk.kind === 'mystery') {
        // 🎁 미스터리 박스: 열어보기 전엔 모른다 — 다른 아이템 하나가 즉석에서 발동(폭탄일 수도!)
        const pool = ['hammer', 'mush', 'star', 'bolt', 'bomb', 'gust']
        const got = pool[Math.floor(state.rng() * pool.length)]
        pushFeed(state, 'obj', `🎁 ${emojiOf(h.zodiacId)} ${h.name} — 상자에서 ${({ hammer: '🔨', mush: '🍄', star: '⭐', bolt: '⚡', bomb: '💣', gust: '💨' })[got]}이(가) 나왔다!`)
        state.brawlPickups.push({ id: state.nextId++, kind: got, x: h.x, z: h.z, t: 17.5 }) // 즉시 재습득되는 임시 픽업(0.5초 내 본인 접촉)
  ...
  for (const h of state.heroes) {
    ...
    if (h.respawnT > 0 || h.hp <= 0) { h.brawlBombT = 0.001; h.brawlBombDead = true } // 든 채 죽으면 그 자리에서 즉시 폭발
    h.brawlBombT -= dt
    if (h.brawlBombT > 0) {
      if (h.brawlBombCd > 0) continue
      for (const e of state.heroes) { // 몸으로 옮기기 — 받은 쪽은 잠깐 반환 불가
        if (e === h || e.respawnT > 0 || e.hp <= 0 || e.fallT > 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0) continue
      ...
      // 펑! — 든 사람은 확정 처형, 주변은 대광역+초강넉백. 죽은 채 터져도 그 자리에서.
      h.brawlBombT = 0
  ...
  for (const h of state.heroes) {
    ...
    if ((h.brawlShadowN || 0) > 0) { // 🗡️ 그림자 연무 집행: 한 명씩, 팍-팍-팍
      if (h.hp <= 0 || h.respawnT > 0) h.brawlShadowN = 0
      else {
        h.brawlShadowT -= dt
        if (h.brawlShadowT <= 0) {
          let mk = null
          ...
          for (const e of state.heroes) {
            ...
            const ed = Math.hypot(h.x - e.x, h.z - e.z)
            if (ed > 15) continue
            const seen = h.brawlShadowHit.includes(e.id)
            const score = ed + (seen ? 100 : 0) // 아직 안 벤 표적 우선 — 다 벴으면 재타격 허용
            if (score < mkD) { mkD = score; mk = e }
          }
          if (!mk) h.brawlShadowN = 0
          else {
    ...
    if (h.brawlChainT > 0) { // ⛓️ 사슬 회오리 집행: 모아둔 전원을 사방으로 대투척
      ...
      if (h.brawlChainT === 0 && h.hp > 0 && h.respawnT <= 0) { // ⚖️ 단죄 집행 — 모인 전원에게 심판의 일격
        for (const e of state.heroes) {
          if (e.team === h.team || e.respawnT > 0 || e.hp <= 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0 || (e.brawlMushT || 0) > 0) continue
          const ed = Math.hypot(h.x - e.x, h.z - e.z)
          if (ed > 7) continue
    ...
    if (h.brawlStarT > 0) h.brawlStarT = Math.max(0, h.brawlStarT - dt)
    // 방관 카운터: 전투(가해·피격) 없이 버티면 누적 — 멀찍이 어부지리 전략을 막는다.
    //  12초+ = 봇들의 사냥감(표적 가중치), 18초+ = 겁쟁이 세금(서서히 힘이 빠진다)
    if (h.respawnT > 0 || h.hp <= 0 || h.fallT > 0) {
      h.brawlIdleT = 0
  ...
  // 낙하 진행(콜로세움과 같은 연출) — 다 떨어지면 사망 확정. 넉백 가해자에게 킬 크레딧(lastHitBy).
  for (const h of state.heroes) {
...
function stepHero(state, h, dt) {
  ...
  if (h.beamDeathAt && state.time >= h.beamDeathAt) {
    h.beamDeathAt = null
  ...
  // 섬멸의 광선 비행: 지형·충돌을 전부 무시하고 빛의 방향으로 맵 끝까지 쏘아진다 —
  //  resolveTerrain을 아예 안 태워 벽·타워를 그대로 관통한다(정말 "섬멸당하는" 그림).
  if (h.beamFlingT > 0) {
    h.beamFlingT = Math.max(0, h.beamFlingT - dt)
    const W = state.map.WORLD
    h.x = Math.max(W.minX, Math.min(W.maxX, h.x + h.beamFlingVx * dt))
    h.z = Math.max(W.minZ, Math.min(W.maxZ, h.z + h.beamFlingVz * dt))
  }
  ...
  if (h.stunT <= 0 && h.rootT <= 0 && h.knockT <= 0 && !(h.charmT > 0) && !(h.fallT > 0) && dreamCharmed(state, h)) {
    ...
    if (prey) {
      ...
      if (ad > heroRange(h) * 0.8) { // 사거리에 들 때까지만 걸어간다
        ...
        h.z += Math.sin(h.dir) * sp * dt
      }
    }
    h.mx = 0 // 조작 입력은 먹지 않는다
    h.mz = 0
  }
...
function stepProjectiles(state, dt) {
  ...
  for (const p of state.projectiles) {
    ...
    if (p.kind === 'swordwave') {
      ...
      p.x += p.vx * dt
      p.z += p.vz * dt
      p.travel += (p.spd || SWORDWAVE_SPEED) * dt
      ...
      if (p.travel >= (p.range || SWORDWAVE_RANGE)) remove.add(p.id)
      continue
    }
...
const HUNT_CD = 26 // 쿨 ±2s 탐색은 노이즈(±10pp)에 파묻힌다 — 26/30이 실측 데이터가 가장 두터운 지점
...
// ── 발구르기(카르곤 시그니처): 크게 도약했다 대지에 내리꽂는다 — 착지 순간 전장이 통째로
//  흔들리고, 안전지대(초록 원) 밖 전원 피해 + 공중으로 붕 뜬다. "빠져"(장판)의 반대 문법:
//  "여기로 피해!" — 초록 원 하나가 유일한 답이라 읽기 쉽고, 거인의 힘이 화면 전체로 과시된다.
//  (1차 구현이던 넉백+용암 웅덩이는 개연성이 없고 밀려나는 것도 심심했다 — 유저 피드백으로 개편)
export const SLAM_LEAP_T = 2.3 // 도약~착지(예고) — 안전지대까지 달려갈 시간. 티어 tele 적용
//  (1.9초 + 무작위 방위 안전지대는 봇이 못 닿아 클리어율 38%→19% 폭락 — 팀 쪽 생성+2.3초로 복원)
export const SLAM_SAFE_R = 6 // 안전지대 반경 — 5인이 모여 설 크기. 씬(초록 원)과 공유
const SLAM_R = 34 // 착지 충격 반경(전장급) — 이 밖은 전투 밖이라 무관
const SLAM_CD = 21 // 도약 2.3초 동안 보스가 쉬는 비용을 주기로 되찾는다(26초는 클리어율 69%로 과속 완화)
const SLAM_AIR = 1.0 // 휘말리면 공중에 뜨는 시간(돌풍 airT 재사용)

// ── 아르케인 개편 킷: 소환 의식(소환석 DPS 체크) + 바위감옥·운석 연계 + 섬멸 광선 ──
//  카르곤=위치 판단, 녹스=시선, 아르케인=철거 우선순위(DPS 체크) — 축을 완전히 가른다.
...
//  (2.4초는 너무 느려 긴장감이 없다는 피드백 — 절반으로. 즉사기지만 "반사신경 반 박자"면 피한다)
const BEAM_CD = 34
...
export const CONE_HALF = 0.55 // 반각(라디안, 약 31.5°)
const CONE_CHARGE = 1.1
const CONE_CD = 17
const GALE_CD = 20 // 마력 회오리(돌풍술사 크게) — 움직이는 위험물
...
function bossFan(state, h, foe) {
  const dir = Math.atan2(foe.z - h.z, foe.x - h.x)
  h.dir = dir
  h.bossFanSeq = (h.bossFanSeq || 0) + 1 // 씬: 부채꼴 공격 = 항상 횡으로 크게 쓸기 모션
...
function bossThink(state, h, dt) {
  ...
  if (h.bossGazeAt && state.time >= h.bossGazeAt) {
    ...
    h.gimRestUntil = state.time + 4 // 야바위와 겹치지 않게 — 기믹은 한 번에 하나
  }
  ...
  if (h.huntUntil) {
    ...
    if (!mark || mark.respawnT > 0) {
      ...
      // 포위 활주: 본체·보조 그림자가 각자 다른 방위의 근접점(표적 곁 1.2)으로 조여든다 — 겹치지 않는다.
      //  보조는 본체보다 한 발 느리다 — 포위 압박은 주되 확정 사망은 안 만든다(실행하면 산다)
      const glide = (o, phi, spd) => {
        const tx = mark.x + Math.cos(phi) * 1.2
        const tz = mark.z + Math.sin(phi) * 1.2
        const d = Math.hypot(tx - o.x, tz - o.z) || 0.001
  ...
  // 어둠의 정적: 암전 속 랜덤 대기 → 등장(전원 공개)·직선 경고 0.8초 → 실제 고속 돌진 — 페이즈별 1/2/3단 연속.
  ...
  if (h.stillVanishAt != null) {
    ...
    // ② 발사: 예고가 다 되면 실제 고속 돌진 개시(순간이동이 아니다).
    //  dashHits는 시전 전체(다단·연쇄 포함)로 유지 — 다단 간격(1.12s)이 공포(1.6s)보다 짧아
    //  같은 사람이 제어를 잃은 채 연타당하는 히트락을 막는다(한 시전에 한 번만 아프다)
    if (h.stillWarned && h.dashRunT == null && h.stillVanishAt != null && state.time >= h.stillVanishAt) {
      h.dashRunT = 0
      h.dashFromX = h.x
      h.dashFromZ = h.z
      h.dashHits = h.dashHits || []
      h.dashKilled = false
      h.dashGoSeq = (h.dashGoSeq || 0) + 1 // 씬 발사 섬광·진동 트리거
      pushFxDir(state, 'dash', h.x, h.z, NOXDASH_LEN, h.dashDir, h.team)
      pushFx(state, 'shriek', h.x + Math.cos(h.dashDir) * 4, h.z + Math.sin(h.dashDir) * 4, 6, h.team, 1.0)
    }
    // ③ 돌진 진행: 0.32초에 직선 42를 주파 — 지나온 경로 위의 적을 그때그때 강타
    if (h.dashRunT != null) {
      h.dashRunT += dt
      const frac = Math.min(1, h.dashRunT / STILL_DASH_RUN)
      const bx = Math.cos(h.dashDir)
      const bz = Math.sin(h.dashDir)
      h.x = h.dashFromX + bx * NOXDASH_LEN * frac
      h.z = h.dashFromZ + bz * NOXDASH_LEN * frac
      h.revealT = Math.max(h.revealT || 0, 0.4)
      for (const e of state.heroes) {
        if (e.team === h.team || e.respawnT > 0 || e.isBoss || h.dashHits.includes(e.id)) continue
        const rx = e.x - h.dashFromX
        const rz = e.z - h.dashFromZ
        const along = rx * bx + rz * bz
        const perp = Math.abs(-rx * bz + rz * bx)
      ...
      if (frac >= 1) {
        // 단 완료 — 킬이 났으면 단수를 페이즈 단수로 리셋해 다시 시작, 아니면 한 단 소모
        state.map.resolveTerrain(h, 2.2, colliders(state))
        h.dashRunT = null
  ...
  if (h.bossSlamAt && state.time >= h.bossSlamAt) {
    h.bossSlamAt = null
    h.bossSlamSeq = (h.bossSlamSeq || 0) + 1
    pushFx(state, 'quake', h.x, h.z, 14, h.team, 1.3)
    pushFx(state, 'rocksplash', h.x, h.z, 10, h.team, 1.0)
    const safe = h.bossSlamSafe
    let hit = 0
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
    ...
    h.bossSlamSafe = null
    pushFeed(state, 'obj', hit
      ? `💥 대지가 갈라진다 — ${hit}명이 충격에 떠올랐다!`
      : '💥 대지가 갈라진다 — 모두 안전지대로 피했다!')
  }
  // 도약 중: 하늘에 떠 있다 — 이동·다른 기술 없음(착지가 곧 기술이다)
  if (h.bossSlamAt) {
    h.mx = 0
    h.mz = 0
    return
  }
  // 소환 의식(아르케인): 의식 중 무적 채널(bossInvuln) — 소환석이 유일한 표적이 된다.
  //  시간 안에 전부 부수면 의식이 역류해 그로기(저지 보상 딜 타임), 남기면 남은 수만큼 즉결 처형.
  if (h.stonesAt != null) {
    const alive = state.minions.filter((m) => m.stone && m.hp > 0)
    if (!alive.length) {
      h.stonesAt = null
      h.bossGroggyT = 2.2 // 역류 그로기(CC 저항 무시) — 부순 보람이 손에 잡히는 보상
      h.stunT = Math.max(h.stunT, 1) // 💫 표시용
      pushFx(state, 'berserk', h.x, h.z, 8, h.team, 1.0)
      pushFeed(state, 'obj', `✨ 소환석 전파괴 — 역류한 마력에 ${h.name}이(가) 휘청인다`)
    } else if (state.time >= h.stonesAt) {
      h.stonesAt = null
      const pool = state.heroes.filter((e) => e.team !== h.team && e.respawnT <= 0 && !e.isBoss)
      let executed = 0
      for (const m of alive) {
        state.minions = state.minions.filter((o) => o !== m)
        pushFx(state, 'meteorhit', m.x, m.z, 3, 'red', 0.8)
      ...
      pushFeed(state, 'obj', executed
        ? `💀 소환 의식 완성 — ${executed}명이 마력에 소멸했다!`
        : '💀 소환 의식이 완성되었다')
    } else {
      h.mx = 0
      h.mz = 0
      return // 채널 — 무적으로 힘을 모은다(이동·평타·기술 없음)
    }
  }
  // 섬멸의 광선 집행: 경고 직사각(길이 BEAM_LEN × 폭 2·BEAM_HALF) 안의 적 전원 —
  //  빛에 밀려 날아가고(넉백) 반 박자 뒤 소멸(즉사). 경로에서 비켰다면 완전 회피.
  if (h.bossBeamAt && state.time >= h.bossBeamAt) {
    h.bossBeamAt = null
    h.bossBeamSeq = (h.bossBeamSeq || 0) + 1
    for (const off of beamOffsets(h.bossBeamN || 1)) {
      const bd = h.bossBeamDir + off
      const bx = Math.cos(bd)
      const bz = Math.sin(bd)
      for (const e of state.heroes) {
        if (e.team === h.team || e.respawnT > 0 || e.isBoss || e.beamFlingT > 0) continue
        const rx = e.x - h.x
        const rz = e.z - h.z
        const along = rx * bx + rz * bz
        const perp = Math.abs(-rx * bz + rz * bx)
        if (along < 1 || along > BEAM_LEN || perp > BEAM_HALF) continue
        // 빛의 격류에 쏘아진다 — 지형·충돌을 전부 무시하고 광선 방향으로 맵 끝까지(stepHero 비행 처리)
        e.beamFlingT = BEAM_FLING_T
        e.beamFlingVx = bx * BEAM_FLING_V
        e.beamFlingVz = bz * BEAM_FLING_V
        e.stunT = Math.max(e.stunT, BEAM_FLING_T) // 비행 중 조작 불능
        e.beamDeathAt = state.time + BEAM_FLING_T // 날아가던 끝에서 소멸
        e.beamDeathFrom = h.id
      }
      for (let k = 1; k <= 4; k++) pushFx(state, 'meteorhit', h.x + bx * k * 12, h.z + bz * k * 12, 7, h.team, 0.9)
    }
  }
  // 광선 조준 중: 그 방향을 겨눈 채 미동도 없다 — 경고 띠가 곧 데드라인
  if (h.bossBeamAt) {
    h.dir = h.bossBeamDir
    h.mx = 0
    h.mz = 0
  ...
  // 서리 숨결 집행: 부채꼴(CONE_R·반각 CONE_HALF) 안 전원 피해 + 깊은 빙결
  ...
  // 그로기(기믹 보상 딜 타임): CC 저항(BOSS_CC_RESIST)을 받지 않는 전용 경직 —
  //  소환석 저지·야바위 정답의 보상 시간이 저항에 갉아먹히면 부순 보람이 사라진다
  if ((h.bossGroggyT || 0) > 0) {
    h.bossGroggyT = Math.max(0, h.bossGroggyT - dt)
    h.mx = 0
    h.mz = 0
    return
  }
  ...
  if (!h.bossMarching) {
    h.bossMarching = true
    pushFeed(state, 'obj', `👹 ${CLASSES[h.cls].name}이(가) 진군을 시작했다`)
  }
...
function bossColossus(state, h, foe) {
  ...
  // 발구르기(시그니처 · 보스전 전용): 하늘로 크게 도약 → 안전지대(초록 원) 예고 → 착지 쿵.
  //  안전지대 밖 전원 피해 + 공중 띄움. 무한방어의 메아리 보스는 쓰지 않는다(모드 축 분리).
  if (!h.defenseBoss && (h.bossCd.stomp ?? 1) <= 0 && nearFoe && !h.bossSlamAt) {
    h.bossCd.stomp = SLAM_CD * cdMul
    const tele = SLAM_LEAP_T * bossTierOf(state).tele
    h.bossSlamAt = state.time + tele
    h.bossSlamT0 = tele // 씬 도약 포물선 정규화용
    // 안전지대 하나 — "모두 같은 곳으로"가 읽기 쉽다. 적팀 무게중심 쪽(±흔들림)에 만든다:
    //  전장 반대편에 떨어지면 근접·원거리 어느 쪽도 시간 안에 못 가는 억울함이 생긴다.
    let cx = 0
    let cz = 0
    let nn = 0
    for (const e of state.heroes) {
    ...
    const base = nn ? Math.atan2(cz / nn - h.z, cx / nn - h.x) : state.rng() * Math.PI * 2
    const a = base + (state.rng() - 0.5) * 1.2
    const d = (p === 3 ? 12 : 9) + state.rng() * 4
    h.bossSlamSafe = { x: h.x + Math.cos(a) * d, z: h.z + Math.sin(a) * d }
    pushBossZone(state, h, {
      x: h.bossSlamSafe.x, z: h.bossSlamSafe.z, r: SLAM_SAFE_R, delay: tele,
  ...
  if (h.bossCd.a <= 0 && nearFoe) {
    ...
    h.bossSmashSeq = (h.bossSmashSeq || 0) + 1 // 대지 강타 = 종으로 내리찍기 모션
  ...
  if (h.bossCd.d <= 0 && foe && dist(h, foe) < 18) {
    h.bossCd.d = 18 * cdMul
    h.bossFanSeq = (h.bossFanSeq || 0) + 1 // 부채꼴 = 횡으로 크게 쓸기 모션
...
function bossArchmage(state, h, foe) {
  ...
  if (!h.defenseBoss && (h.bossCd.stones ?? 1) <= 0
    ...
    // 실패→처형→전멸의 사망 나선이 돌던 문제를 구조적으로 끊는다(5인 9 → 3인 6 → 2인 5)
    const aliveN = state.heroes.filter((e) => e.team !== h.team && e.respawnT <= 0 && !e.isBoss).length
    const stoneHp = Math.max(4, Math.min(STONE_HP, Math.round(2 + aliveN * 1.4)))
    for (let i = 0; i < count; i++) {
      const a = (i / count) * Math.PI * 2 + state.rng() * 0.9
      const d = 8 + state.rng() * 5
      state.minions.push({
        id: state.nextId++, team: h.team, stone: true, lane: 'mid', ranged: false,
        x: h.x + Math.cos(a) * d, z: h.z + Math.sin(a) * d,
        hp: stoneHp, maxHp: stoneHp, atkCd: 0, wpI: 0, dir: 0, atkSeq: 0,
      })
    }
  ...
  //  빙결·속박에 묶인 채 즉사기가 날아오면 정답이 없다(시뮬 0/16 교훈 재적용) ──
  const bigReady = state.time >= (h.comboRestUntil || 0)
  // 궁극기 '섬멸의 광선'(빛의 화살): 전방 긴 직사각 경고 → 경로의 모든 것을 불태운다.
  //  맞으면 무조건 즉사(빛에 밀려 날아가 소멸) — 경로에서 비키는 것만이 답. 집행은 펜딩 섹션.
  if (!h.defenseBoss && h.bossCd.d <= 0 && !h.bossBeamAt && bigReady) {
    // 조준: 리쉬와 무관하게 보이는 가장 가까운 적 — 광선은 사거리가 전장급이라 스스로 표적을 찾는다
    let bt = null
    let bd2 = 40 * 40
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
    ...
    if (bt) {
      ...
      h.bossBeamAt = state.time + charge
      h.bossBeamT0 = charge
      h.dir = h.bossBeamDir
  ...
  // 밀쳐내기(돌풍술사, 크게): 몸에 붙은 적을 염력으로 강하게 밀쳐낸다 — 근접 순삭 차단.
  //  예고 0.8초를 보고 미리 빠지면 안 밀리고 안 맞는다.
  if (h.bossCd.c <= 0) {
    let near = 0
    for (const e of state.heroes) {
  ...
  // 바위감옥 + 운석 3연타(대지술사+마법사 연계): "가장 뭉친 무리"의 한복판을 넓은 돌기둥 링으로
  //  통째로 가두고, 유저 운석의 몇 배짜리 대형 운석을 세 번 내리꽂는다. 감옥은 걸어서 못 나간다 —
  //  돌진·점멸로 탈출하거나 치유·보호막으로 버텨라(트인 카운터플레이 둘).
  if (h.bossCd.a <= 0 && bigReady) {
    // 군집 타게팅: 반경 PRISON_R 안 이웃이 가장 많은 적 — 최소 1명, 뭉쳤다면 여럿이 갇힌다
    let mark = null
    let best = -1
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
      ...
      let n = 0
      for (const o of state.heroes) {
        if (o.team === h.team || o.respawnT > 0 || o.isBoss) continue
        if (dist2(e, o) <= (PRISON_R - 1) ** 2) n++
      }
    ...
    if (mark) {
      ...
      for (const e of state.heroes) { // 링 안 전원 잠깐 속박 — 최소 표적 포함 갇힘을 보장
        if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
        if (dist2(e, mark) > PRISON_R * PRISON_R) continue
        e.rootT = Math.max(e.rootT, 0.9)
        caged++
      }
      for (let i = 0; i < 3; i++) {
        state.zones.push({
  ...
  if (h.bossCd.b <= 0 && foe && dist(h, foe) < CONE_R - 2 && !h.bossConeAt && bigReady) {
    ...
    h.bossConeT0 = charge
    h.comboRestUntil = state.time + charge + 2.2
    h.dir = h.bossConeDir
    return
  }
  // 마력 회오리(돌풍술사 크게): 대형 회오리 2~3기가 부챗살로 굴러간다 — 닿으면 피해+공중 띄움.
  //  정적인 장판이 아니라 "움직이는 위험" — 굴러오는 궤적을 읽고 사이로 빠져야 한다.
  if (h.bossCd.fan <= 0 && foe && dist(h, foe) < 30) {
...
function bossShadow(state, h, foe, siege) {
  ...
  //  (지옥에서 습격·신월참까지 빨라지면 4%까지 추락 — 밴드 밖)
  ...
  if (!h.defenseBoss && (h.bossCd.hunt ?? 1) <= 0 && !h.huntUntil && state.time >= (h.gimRestUntil || 0)) {
    ...
    if (pool.length) {
      ...
      mark.huntFrom = h.id
      pushFx(state, 'blink', h.x, h.z, 4, h.team, 0.8)
      pushFeed(state, 'obj', shadowN > 1
        ? `☠️ 죽음의 그림자 ${shadowN}갈래가 ${mark.name}을(를) 뒤쫓는다`
        : `☠️ 죽음의 그림자가 ${mark.name}을(를) 뒤쫓는다`)
      return
    }
  }
  ...
  if (!h.defenseBoss && (h.bossCd.gaze ?? 1) <= 0 && !h.bossGazeAt && state.time >= (h.gimRestUntil || 0)) {
    let near = 0
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.isBoss) continue
  ...
  if (h.bossCd.a <= 0) {
    ...
    if (weakest) {
      ...
      pushBossZone(state, h, {
        x: weakest.x, z: weakest.z, r: 4.2, delay: 1.0, dmg: skillDmg(h, 240, 4.8),
  ...
  if (h.bossCd.b <= 0) {
    ...
    if (near >= (p === 3 ? 1 : 2)) {
      ...
      pushBossZone(state, h, {
        x: h.x, z: h.z, r: roarR, delay: 1.2, dmg: skillDmg(h, 170, 3.2), fear: 1.6,
...
function botHuntFlee(state, h) {
  ...
  let ax = 0
  let az = 0
  for (const sh of b ? [b, ...(b.huntShadows || [])] : []) {
    const d = Math.max(1, dist(h, sh))
    ax += (h.x - sh.x) / (d * d)
    az += (h.z - sh.z) / (d * d)
  }
  const a = ax || az ? Math.atan2(az, ax) : h.dir
...
// 서리 숨결(부채꼴 예고)도 같은 반사신경: 부채꼴 밖 각도로 벗어난다.
function botDodgeBeam(state, h, dt) {
  for (const b of state.heroes) {
    if (!b.isBoss || b.team === h.team || b.respawnT > 0) continue
    if (b.bossBeamAt) {
      const n = b.bossBeamN || 1
      let inLane = false
      for (const off of beamOffsets(n)) {
        const bd = b.bossBeamDir + off
        const bx = Math.cos(bd)
        const bz = Math.sin(bd)
        const rx = h.x - b.x
        const rz = h.z - b.z
        const along = rx * bx + rz * bz
        const perp = Math.abs(-rx * bz + rz * bx)
        if (along >= -2 && along <= BEAM_LEN + 2 && perp <= BEAM_HALF + 2.2) { inLane = true; break }
      }
      if (inLane) {
        // 가닥 "사이"의 안전 각도로 빠진다 — 가까울수록 골목이 좁으니 바깥으로도 나간다
        const gaps = n >= 3 ? [-1.7, -0.4, 0.4, 1.7] : n === 2 ? [-1.35, 0, 1.35] : [-Math.PI / 2, Math.PI / 2]
        let rel = Math.atan2(h.z - b.z, h.x - b.x) - b.bossBeamDir
        while (rel > Math.PI) rel -= 2 * Math.PI
        while (rel < -Math.PI) rel += 2 * Math.PI
        let g = gaps[0]
        for (const gg of gaps) if (Math.abs(gg - rel) < Math.abs(g - rel)) g = gg
        const esc = b.bossBeamDir + g
        const d = Math.max(16, dist(h, b) + 2)
        steerToward(state, h, { x: b.x + Math.cos(esc) * d, z: b.z + Math.sin(esc) * d })
        botAttack(state, h, dt)
        return true
      }
    }
...
// 소환석 철거: 의식(무적 채널) 중엔 보스가 아니라 소환석이 유일한 표적 — 타격 횟수가 곧 진행도.
// 가장 가까운 돌부터 부순다(흩어져 각개 철거 — 몰려다니면 시간 안에 못 끝낸다).
function botBreakStones(state, h, dt) {
  let ritual = false
  for (const e of state.heroes) {
    if (e.isBoss && e.team !== h.team && e.stonesAt != null) { ritual = true; break }
  }
  if (!ritual) return false
  let best = null
  let bd = Infinity
  for (const m of state.minions) {
    if (!m.stone || m.hp <= 0) continue
    const d2v = dist2(h, m)
    if (d2v < bd) { bd = d2v; best = m }
  }
  if (!best) return false
...
// 발구르기 감지: 도약(예고) 중이면 안전지대(초록 원)로 피신 — 달려가면서도 계속 때린다.
// 이미 원 안이면 하던 일 계속(최소 시간 원칙). 전투 밖(SLAM 반경 밖)이면 무관.
function botSeekSafe(state, h, dt) {
  let boss = null
  for (const e of state.heroes) {
    if (e.isBoss && e.team !== h.team && e.bossSlamAt && e.bossSlamSafe && e.respawnT <= 0) { boss = e; break }
  }
  if (!boss || dist(h, boss) > 36) return false
  const safe = boss.bossSlamSafe
  if (Math.hypot(h.x - safe.x, h.z - safe.z) <= SLAM_SAFE_R - 1.2) return false // 이미 안 — 계속 싸운다
  steerToward(state, h, safe)
  botAttack(state, h, dt)
  return true
}
...
function botDodgeBossZone(state, h, dt) {
  ...
  for (const z of state.zones) {
    ...
    if (z.safe) continue // 안전지대(발구르기)는 피할 곳이 아니라 갈 곳 — botSeekSafe가 안내한다
    // ① 예고 중인 장판 위 → 터지기 전에 이탈 ② 잔류 도트 장판(용암·서리)을 밟고 서 있으면 걸어 나온다
    //  (잔류를 모른 척하면 서리밭·용암에서 체력이 줄줄 샌다 — 시뮬 44%→25% 폭락 교훈)
...
function brawlBotDuty(state, h, dt) {
  ...
  if ((h.brawlBombT || 0) > 0) {
    ...
    let nd = 1e9
    for (const e of state.heroes) {
      if (e.team === h.team || e.respawnT > 0 || e.hp <= 0 || e.fallT > 0 || (e.brawlGuardT || 0) > 0 || (e.brawlStarT || 0) > 0) continue
      const ed = dist(h, e)
  ...
  if (h.hp > h.maxHp * 0.45 && state.brawlPickups.length) {
    let pk = null
    let pd = 14
    for (const o of state.brawlPickups) {
      const od = Math.hypot(h.x - o.x, h.z - o.z)
      if (od < pd) { pd = od; pk = o }
    }
    if (pk) { steerToward(state, h, pk); botAttack(state, h, dt); return }
  }
  const d = dist(h, foe)
  const range = CLASSES[h.cls].range
  ...
  if ((frail && d < 13) || nearEdge) {
    ...
    // 원거리 카이팅: 사거리 끝으로 물러선다 — 단 등 뒤가 낭떠러지면 중앙 쪽으로
    const dx = (h.x - foe.x) / (d || 0.001)
    const dz = (h.z - foe.z) / (d || 0.001)
    const back = { x: h.x + dx * (range * 0.85 - d), z: h.z + dz * (range * 0.85 - d) }
    goal = Math.hypot(back.x, back.z) > state.brawlR - 3 ? { x: h.x * 0.5, z: h.z * 0.5 } : back
  } else {
    goal = null // 사거리 안: 제자리 공방
  }
  if (goal) steerToward(state, h, goal)
  else { h.mx = 0; h.mz = 0 }
  botAttack(state, h, dt)
  if (d < range + 6) {
    const nearCount = state.heroes.filter((e) => e.team !== h.team && e.respawnT <= 0 && dist(h, e) < 12).length
...
export function makeView(state) {
  ...
  return {
    ...
    heroes: state.heroes.map((h) => ({
      ...
      //  bossLeapT(+T0): 발구르기 도약 잔여/총시간 — 포물선 도약 연출 · bossSlamSeq: 착지 → 화면 진동
      ...
      ...(h.isBoss ? {
        ...
        bossSlamSeq: h.bossSlamSeq || 0,
        bossFanSeq: h.bossFanSeq || 0,
        bossSmashSeq: h.bossSmashSeq || 0,
        // 아르케인: 소환 의식 잔여(>0 = 무적 채널·소환석 카운트다운) + 섬멸 광선 조준(경고 띠)
        stonesT: r2d(Math.max(0, (h.stonesAt || 0) - state.time)),
        bossBeamT: r2d(Math.max(0, (h.bossBeamAt || 0) - state.time)),
        bossBeamT0: r2d(h.bossBeamT0 || 0),
        bossBeamDir: r2d(h.bossBeamDir || 0),
        bossBeamN: h.bossBeamN || 1, // 국면별 방사 가닥 수(1/2/3)
        bossBeamSeq: h.bossBeamSeq || 0,
