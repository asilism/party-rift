...
function stepBots(state, dt) {
  // 보스전: 전선 위협 여부(틱당 1회) — 새 오브젝트(용/이무기) 결심을 막는 데 쓴다.
  //  단 이미 결심한(커밋된) 오브젝트는 전선 위협과 무관하게 끝까지 밀어붙인다.
  const frontHot = state.mode === 'boss' ? bossFrontThreat(state) : false
  // 팀 콜(오브젝트 집결/구조물 수비)은 틱마다 새로 계산한다.
  //  allowNew=false면 새로 시작은 안 하되 진행 중인 커밋은 유지한다.
  const objCall = {
    blue: computeObjectiveCall(state, 'blue', !(state.mode === 'boss' && frontHot)),
