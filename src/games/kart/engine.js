...
export function createGame(players, rng = Math.random, trackId = DEFAULT_TRACK_ID) {
  ...
  const karts = players.map((p, i) => {
    ...
    return {
      ...
      fallT: 0, // 추락(용암 풍덩/기차 펑) 연출 남은 시간
      fallKind: null, // 'pool'(풍덩) | 'train'(기차에 치여 날아감)
      tossX: 0, // 기차에 치였을 때 튕겨나가는 속도
      tossZ: 0,
...
function stepBots(state, dt) {
  ...
  for (const k of state.karts) {
    ...
    // 건널목 앞: 기차가 보이면 봇도 끽 멈춰 선다
    for (const tr of track.trains || []) {
      const ahead = wrapDelta(tr.i - k.ci, track.n)
      if (ahead <= 0 || ahead > 26) continue
      if (trainCars(track, tr, state.time).some((c) => Math.abs(c.lat) < track.halfW + 14)) {
        k.brake = true
        break
      }
    }
...
function stepObstacles(state, dt) {
  ...
  track.obstacles.forEach((ob, idx) => {
    ...
    for (const k of state.karts) {
      ...
      k.bumpSeq++
      k.bumpKind = ob.kind
      if (ob.kind === 'cactus' || ob.kind === 'snowman' || ob.kind === 'barrier') {
      ...
      if (ob.kind === 'cactus' || ob.kind === 'magma' || ob.kind === 'barrier') {
        stunKart(k, 1.0) // 따갑거나, 앗 뜨겁거나, 공사중이거나!
      } else if (ob.kind === 'tornado' || ob.kind === 'steam') {
...
function stepKart(state, k, dt) {
  const track = state.track
  // 추락 연출: 용암/강물엔 가라앉고, 기차에 치이면 하늘로 펑 날아갔다 떨어진다.
...
function stepTrains(state) {
  const track = state.track
  if (!track.trains?.length) return
  for (const tr of track.trains) {
    const cars = trainCars(track, tr, state.time)
    if (!cars.length) continue
    const s = track.samples[tr.i]
    for (const k of state.karts) {
      if (k.finished || k.rocketT > 0 || k.flyT > 0 || k.invT > 0 || k.air || k.fallT > 0) continue
      for (const c of cars) {
        const dx = k.x - c.x
        const dz = k.z - c.z
        const r = tr.r + 1.1
        ...
        k.fallKind = 'train'
        k.fallSeq++
        k.bumpSeq++ // (배너는 fallSeq가 우선하지만 통계용으로 함께 센다)
        k.bumpKind = 'train'
        k.air = false
        k.vy = TRAIN_TOSS_UP
        k.tossX = s.nx * tr.dir * TRAIN_TOSS_SIDE
        k.tossZ = s.nz * tr.dir * TRAIN_TOSS_SIDE
        k.speed = 0
        k.boostT = 0
        releaseDrift(k, false)
        k.respawnI = (((tr.i - RESPAWN_BACK) % track.n) + track.n) % track.n
        break
      }
    }
  }
...
export function makeView(state) {
  ...
  return {
    ...
    // 기차가 건널목에 접근/통과 중 (경적 효과음 트리거)
    trainNear: (state.track.trains || []).some((tr) =>
      trainCars(state.track, tr, state.time).some(
        (c) => Math.abs(c.lat) < state.track.halfW + 24
      )
    ),
